      ...
      try {
        ...
        if (response.ok) {
          ...
          const badges: BadgeItem[] = data.badges.map((mint: any) => ({
            id: mint.id,
            name: mint.badgeName?.includes('EdgeEsmeralda') || mint.badgeType === 'EdgeEsmeralda' ? 'EdgeEsmeralda Badge' : mint.badgeName,
            description: (mint.badgeName?.includes('EdgeEsmeralda') || mint.badgeType === 'EdgeEsmeralda')
              ? 'First badge earned for completing the onboarding quiz and Attending EdgeEsmeralda'
              : `Badge for ${mint.badgeName}`,
            image: (mint.badgeName?.includes('EdgeEsmeralda') || mint.badgeType === 'EdgeEsmeralda') ? '/badges/Edge_Badge.png' : '/badges/default.png',
        ...
        if (storedMintResult && userBadges.length === 0) {
          try {
            ...
            const fallbackBadge: BadgeItem = {
              id: 'fallback',
              name: 'EdgeEsmeralda Badge',
              description: 'First badge earned for completing the onboarding quiz and Attending EdgeEsmeralda',
              ...
              setTransactions(prev => [
                {
                  id: 1,
                  type: 'mint',
                  description: 'EdgeEsmeralda Badge Minted',
  ...
  const handleAddToMetamask = async () => {
    ...
    try {
      if (typeof window !== 'undefined' && window.ethereum) {
        ...
        const tokenId = '1'; // EdgeEsmeralda token ID
              ...
              >
                ...
                <div className="text-center">
                  <div className="text-sm font-medium text-gray-900 truncate">
                    {badge.name.replace('EdgeEsmeralda ', '')}
                  </div>
